...
urlpatterns = [
    ...
    path('api/<slug:volume_name>/list/<path:subpath>/', views.api_list, name='api_list_subpath'),
    path('api/<slug:volume_name>/download/<path:filepath>/', views.api_download, name='api_download'),
...
if not getattr(django_settings, 'FILECHEST_ADHOC_MODE', False):
    urlpatterns = [
    # login
        path('login/', auth_views.LoginView.as_view(template_name='filechest/login.html'), name='login'),
